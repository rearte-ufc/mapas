class Entity {
	...
	parseRelation(prop, key) {
		const type =
			prop?.["@entityType"] ||
			this.$RELATIONS[key]?.targetEntity?.toLocaleLowerCase();
	...
	populateFiles(files) {
		...
		for (let groupName in files) {
			...
			if (group instanceof Array) {
				...
				this.files[groupName] = group.map(
					(data) => new EntityFile(this, groupName, data)
				);
			} else {
	...
	populateMetalists(metalists) {
		...
		for (let groupName in metalists) {
			...
			this.metalists[groupName] = group.map(
				(data) => new EntityMetalist(this, groupName, data)
			);
		}
	}
	...
	catchErrors(res, data) {
		...
		if (res.status >= 500 && res.status <= 599) {
			...
			if (data.error) {
				...
				this.sendMessage(
					message || this.text("erro de validacao"),
					"error"
				);
			}
		...
		if (onlyModifiedFields) {
			for (let key in result) {
				if (
					...
					JSON.stringify(this.__originalValues[key])
				) {
					delete result[key];
				}
			}
	...
	async doCatch(error) {
		...
		return Promise.reject({
			error: true,
			status: 0,
			data: this.text("erro inesperado"),
			exception: error,
		});
	}
	...
	async destroy() {
		...
		try {
			...
			return this.doPromise(res, () => {
				this.sendMessage(
					this.text("entidade removida definitivamente")
				);
				this.removeFromLists();
			});
		} catch (error) {
	...
	async upload(file, { group, description }) {
		...
		try {
			const res = await fetch(this.getUrl("upload"), {
				method: "POST",
				body: data,
			});
	...
	async createMetalist(group, { title, description, value }) {
		...
		try {
			const res = await this.API.POST(this.getUrl("metalist"), {
				group,
				title,
				description,
				value,
	...
	async createAgentRelation(group, agent, hasControl, metadata) {
		try {
			const res = await this.API.POST(
				this.getUrl("createAgentRelation"),
				{ group, agentId: agent.id, has_control: hasControl }
			);
			...
			this.doPromise(res, (agentRelation) => {
				...
				if (response.status == -5) {
					messages.success(
						this.text("solicitação enviada com sucesso")
					);
				}
			});
		} catch (error) {
	...
	async removeAgentRelation(group, agent) {
		...
		try {
			const res = await this.API.POST(
				this.getUrl("removeAgentRelation"),
	...
	async removeAgentRelationGroup(group) {
		this.__processing = this.text(
			"removendo grupo de agentes relacionados"
		);

		try {
			const res = await this.API.POST(
				this.getUrl("removeAgentRelationGroup"),
				{ group }
			);
			this.doPromise(res, (data) => {
	...
	async renameAgentRelationGroup(oldName, newName) {
		this.__processing = this.text(
			"renomeando grupo de agentes relacionados"
		);
		try {
			const res = await this.API.POST(
				this.getUrl("renameAgentRelationGroup"),
				{ oldName, newName }
			);
			this.doPromise(res, (data) => {
	...
	async createSealRelation(seal) {
		...
		try {
			const res = await this.API.POST(this.getUrl("createSealRelation"), {
				sealId: seal.id,
			});
			...
			this.doPromise(res, (r) => {
				const seal = {
					sealId: r.seal.id,
					sealRelationId: r.id,
					singleUrl: Utils.createUrl("sealRelation", "single", [
						r.id,
					]),
					name: r.seal.name,
	...
	async removeSealRelation(seal) {
		...
		try {
			const res = await this.API.POST(this.getUrl("removeSealRelation"), {
				sealId: seal.sealId,
			});
			this.doPromise(res, (data) => {
				let index;
	...
	async changeOwner(ownerId) {
		...
		try {
			const res = await this.API.POST(this.getUrl("changeOwner"), {
				ownerId,
			});
			this.doPromise(res, (data) => {
