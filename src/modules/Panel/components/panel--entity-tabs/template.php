...
<mc-tabs class="entity-tabs" sync-hash>
    ...
    <?php foreach ($tabs as $status => $label) : ?>
        ...
        <mc-tab v-if="showTab('<?= $status ?>')" cache key="<? $status ?>" label="<?= $label ?>" slug="<?= $status ?>">
            ...
            <mc-entities :name="type + ':<?= $status ?>'" :type="type" :select="select" :query="queries['<?= $status ?>']" :limit="50" :order="queries['<?= $status ?>']['@order']" watch-query>
                <template #header="{entities}">
                    <form class="entity-tabs__filters panel__row" @submit="$event.preventDefault();">
                        <slot name="filters">
                            <input type="search" class="entity-tabs__search-input" aria-label="<?= i::__('Palavras-chave') ?>" placeholder="<?= i::__('Buscar por palavras-chave') ?>" v-model="queries['<?= $status ?>']['@keyword']">
                ...
                <template #default="{entities}">
                    ...
                    <slot v-for="entity in entities" :key="entity.__objectId" :entity="entity" :moveEntity="moveEntity">
                        ...
                        <panel--entity-card v-if="entity.__objectType!='registration'" :key="entity.id" :entity="entity" @undeleted="moveEntity(entity, $event)" @deleted="moveEntity(entity, $event)" @archived="moveEntity(entity, $event)" @published="moveEntity(entity, $event)" :on-delete-remove-from-lists="false">
